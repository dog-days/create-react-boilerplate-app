...
class createReducerFile {
  ...
  getReducersInfo(filesPath) {
    ...
    filesPath.forEach(v => {
      ...
      reducers.forEach(r => {
        ...
        reducersInfo.push({
          name: r,
          path: ('./' + relative(cwdPackageJsonConfig.reducersPath, v)).replace(
            /\\/g,
            '/'
          ),
          absolutePath: v,
  ...
  getReducersFunc(content) {
    ...
    if (con) {
      con.forEach(function(data) {
        var re = data.split('function')[1].replace('(', '').replace(/\ /g, '');
        reducers.push(re);
      });
    }
    return reducers;
  ...
  writeReducerFile(reducersInfo, tpls) {
    ...
    reducersInfo.forEach(v => {
      ...
      reducer += tpl.tagsInfo.tagContents['reducer_reducer'].replace(
        /\$\{reducer\}/g,
        v.name
      );
    });
    ...
    content = content.replace(
      tpl.tagsInfo.tagRegex['reducer_reducer'],
      reducer
    );
