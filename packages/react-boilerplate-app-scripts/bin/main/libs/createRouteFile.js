...
class Script {
  ...
  writeRouteFile(routesInfo, tpls) {
    var _this = this, name = 'routes';
    var tpl = tpls[name], content = tpl.contents;
    var im = '', index = '';
    //指定layout是否是第一次
    var state = {};
    ...
    routesInfo.forEach(v => {
      ...
      var routes_match = routes_file_contents.match(
        /layout.*\:.*("|')(.*)("|')/i
      );
      ...
      if (!layout) {
        //获取相对位置，require使用
        var route_require_path = ('./' +
          relative(cwdPackageJsonConfig.routesPath, v.absolutePath)).replace(
          /\\/g,
          '/'
        );
        if (v.name != 'index') {
          im += tpl.tagsInfo.tagContents['require'].replace(
            /\$\{path\}/g,
            route_require_path
          );
        } else {
          index += tpl.tagsInfo.tagContents['index'].replace(
            /\$\{path\}/g,
            route_require_path
          );
        }
        ...
        //获取相对位置，require使用
        var route_require_path = ('./' +
          relative(child_routesPath, v.absolutePath)).replace(/\\/g, '/');
        ...
        if (fs.existsSync(child_routesPath)) {
          ...
          child_routes = child_routes.replace(
            /\/\/routes\/\//g,
            'require("' + route_require_path + '").default, \n\r  //routes//'
          );
