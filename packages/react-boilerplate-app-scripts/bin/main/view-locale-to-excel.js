  ...
  constructor(config) {
    ...
    if (!config) {
      config = {
        ...
        excelSaveDir: `${packageJsonConfig.appLocalePath}/excel` ||
          'src/locale/excel',
      };
    }
    config = Object.assign(
      {},
      {
        saveNotUniqueLangPath: path.resolve(
          `${config.excelSaveDir}/multi-language(file-position).xlsx`
        ),
        saveUniqueLangPath: path.resolve(
          `${config.excelSaveDir}/multi-language(no repetition).xlsx`
        ),
        translatedLangPath: path.resolve(
          `${config.excelSaveDir}/multi-language(translated).xlsx`
        ),
      },
      config
    );
  ...
  */
  generateExcel() {
    ...
    var buffer = xlsx.build([
      { name: 'multi-language', data: this.compareUniqueLangList },
    ]);
